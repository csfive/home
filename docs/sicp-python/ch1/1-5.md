...
除了选择要执行的语句外，控制语句还用于重复。如果我们编写的每一行代码只执行一次，那么编程将是一项非常低效的工作。只有通过重复执行语句，我们才能释放计算机的全部潜力。我们之前已经见过了一种重复形式：一个函数尽管只被定义一次，但可以被多次应用。迭代控制（Iterative control）结构是另一种多次执行相同语句的机制。

思考斐波那契数列，其中每个数都是前两个数的和：

`0, 1, 1, 2, 3, 5, 8, 13, 21, ...`

每个值都是通过重复应用 `sum-previous-two` 规则构建的。第一个和第二个固定为 0 和 1。例如，第八个斐波那契数是 13。

我们可以使用 `while` 语句来枚举 n 项斐波那契数列。我们需要跟踪已经创建了多少个值（`k`），以及第 k 个值（`curr`）及其前身（`pred`）。单步执行此函数并观察斐波那契数如何一个一个地演化，绑定到 curr。
...
测试一个函数就是去验证函数的行为是否符合预期。现在我们的函数语句已经足够复杂，我们需要开始测试我们的功能实现。

测试是一种系统地执行验证的机制。测试通常采用另一个函数的形式，其中包含一个或多个对被测试函数的样例的调用，然后根据预期结果验证其返回值。与大多数旨在通用的函数不同，测试涉及选择特定参数值，并使用它验证函数调用。测试也可用作文档：演示如何调用函数，以及合适的参数值。
...
文档测试（Doctests）：Python 提供了一种方便的方法，可以将简单的测试直接放在函数的文档字符串中。文档字符串的第一行应该包含函数的单行描述，然后是一个空行。参数和行为的详细描述可能如下。此外，文档字符串可能包含调用该函数的交互式会话示例：
...
如果仅想验证单个函数的 doctest 交互，我们可以使用名为 `run_docstring_examples` 的 `doctest` 函数。不幸的是，这个函数调用起来有点复杂。它的第一个参数是要测试的函数；第二个参数应该始终是表达式 `globals()` 的结果，这是一个用于返回全局环境的内置函数；第三个参数 `True` 表示我们想要“详细”输出：所有测试运行的目录。下面，
...
有效测试的关键是在实现新功能后立即编写（并运行）测试。在实现之前编写一些测试也是一种很好的做法，以便在您的脑海中有一些示例的输入和输出。调用单个函数的测试称为单元测试（unit test）。详尽的单元测试是良好程序设计的标志。