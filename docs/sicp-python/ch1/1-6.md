...
随后，名称 `sqrt_update` 解析为这个新定义的函数，它作为参数传递给 `improve` 。在 `improve` 的主体中，我们必须将 `update` 函数（与 `sqrt_update` 绑定）应用于初始猜测为 1 的 x 。这个最终应用程序为 sqrt_update 创建一个环境，该环境以包含的本地框架开始只有 x ，但父框架 `sqrt` 仍然包含 `a` 的绑定。
