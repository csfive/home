...
迭代改进算法从方程的 `guess` 解（推测值）开始，然后重复应用 `update` 函数来改进该猜测，并调用 `close` 比较来检查当前的 `guess` 是否“足够接近”正确值。
...
通过追踪求解的步骤，我们可以看到这个结果是如何计算出来的。首先，使用 `update`、`close` 和 `guess` 的绑定构造 `improve` 的局部帧。在 `improve` 的函数体中，名称 close 绑定到 square_close_to_successor ，这是在 guess 的初始值上调用的。跟踪其余步骤以查看演变为计算黄金比例的计算过程。
