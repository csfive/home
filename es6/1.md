...
:::info
转载自阮一峰老师的 [ES6 入门教程](https://es6.ruanyifeng.com/)，授权为 [署名-非商用许可证](https://creativecommons.org/licenses/by-nc/4.0/)。
:::
...
- ECMAScript 和 JavaScript 的关系是，前者是后者的规格，后者是前者的一种实现，日常可以互换。
- ES6 是 5.1 版以后的 JavaScript 的下一代标准，涵盖了 ES2015、ES2016、ES2017 等等，而 ES2015 则是正式名称。
- Babel 可以将 ES6 代码转为 ES5 代码，采用 `.babelrc` 文件设置转码规则和插件。
- Babel 默认只转换新的 JavaScript 句法（syntax），而不转换新的 API，所以需要使用 core-js 和 regenerator-runtime（后者提供 generator 函数的转码），为当前环境提供一个垫片，也叫做 polyfill。
- 